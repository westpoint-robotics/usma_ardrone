...
#include <usma_ardrone/usma_ardrone_state_controller.h>
...
ArdroneStateController::ArdroneStateController()
{
...
////////////////////////////////////////////////////////////////////////////////
// Load the controller
void ArdroneStateController::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (!velocity_topic_.empty())
  {
    ros::SubscribeOptions ops = ros::SubscribeOptions::create<geometry_msgs::Twist>(
      velocity_topic_, 1,
      boost::bind(&ArdroneStateController::VelocityCallback, this, _1),
  ...
  if (!takeoff_topic_.empty())
  {
    ros::SubscribeOptions ops = ros::SubscribeOptions::create<std_msgs::Empty>(
      takeoff_topic_, 1,
      boost::bind(&ArdroneStateController::TakeoffCallback, this, _1),
      ros::VoidPtr(), &callback_queue_);
  ...
  if (!land_topic_.empty())
  {
    ros::SubscribeOptions ops = ros::SubscribeOptions::create<std_msgs::Empty>(
      land_topic_, 1,
      boost::bind(&ArdroneStateController::LandCallback, this, _1),
      ros::VoidPtr(), &callback_queue_);
  ...
  if (!reset_topic_.empty())
  {
    ros::SubscribeOptions ops = ros::SubscribeOptions::create<std_msgs::Empty>(
      reset_topic_, 1,
      boost::bind(&ArdroneStateController::ResetCallback, this, _1),
      ros::VoidPtr(), &callback_queue_);
  ...
  if (!imu_topic_.empty())
  {
    ros::SubscribeOptions ops = ros::SubscribeOptions::create<sensor_msgs::Imu>(
      imu_topic_, 1,
      boost::bind(&ArdroneStateController::ImuCallback, this, _1),
      ros::VoidPtr(), &callback_queue_);
    imu_subscriber_ = node_handle_->subscribe(ops);

    ROS_INFO_NAMED("ardrone_state_controller", "Using imu information on topic %s as source of orientation and angular velocity.", imu_topic_.c_str());
  }
  ...
  if (!sonar_topic_.empty())
  {
    ros::SubscribeOptions ops = ros::SubscribeOptions::create<sensor_msgs::Range>(
      sonar_topic_, 1,
      boost::bind(&ArdroneStateController::SonarCallback, this, _1),
      ros::VoidPtr(), &callback_queue_);
    sonar_subscriber_ = node_handle_->subscribe(ops);

    ROS_INFO_NAMED("ardrone_state_controller", "Using sonar information on topic %s as source of altitude.", sonar_topic_.c_str());
  }
  ...
  if (!state_topic_.empty())
  {
    ros::SubscribeOptions ops = ros::SubscribeOptions::create<nav_msgs::Odometry>(
      state_topic_, 1,
      boost::bind(&ArdroneStateController::StateCallback, this, _1),
      ros::VoidPtr(), &callback_queue_);
    state_subscriber_ = node_handle_->subscribe(ops);

    ROS_INFO_NAMED("ardrone_state_controller", "Using state information on topic %s as source of state information.", state_topic_.c_str());
  }
  ...
  ros::AdvertiseServiceOptions toggleCam_ops = ros::AdvertiseServiceOptions::create<std_srvs::Empty>(
    toggleCam_topic,
    boost::bind(&ArdroneStateController::toggleCamCallback, this, _1,_2),
    ros::VoidPtr(),
    &callback_queue_);
  ...
  camera_front_subscriber_ = camera_it_->subscribe(
    ...
    boost::bind(&ArdroneStateController::CameraFrontCallback, this, _1),
    ros::VoidPtr(), in_transport);

  camera_bottom_subscriber_ = camera_it_->subscribe(
    cam_bottom_in_topic, 1,
    boost::bind(&ArdroneStateController::CameraBottomCallback, this, _1),
    ros::VoidPtr(), in_transport);
  ...
  ros::SubscribeOptions cam_info_front_ops = ros::SubscribeOptions::create<sensor_msgs::CameraInfo>(
    ...
    boost::bind(&ArdroneStateController::CameraInfoFrontCallback, this, _1),
    ros::VoidPtr(), &callback_queue_);
  ...
  ros::SubscribeOptions cam_info_bottom_ops = ros::SubscribeOptions::create<sensor_msgs::CameraInfo>(
    ...
    boost::bind(&ArdroneStateController::CameraInfoBottomCallback, this, _1),
    ros::VoidPtr(), &callback_queue_);
  camera_info_bottom_subscriber_ = node_handle_->subscribe(cam_info_bottom_ops);

  // callback_queue_thread_ = boost::thread( boost::bind( &ArdroneStateController::CallbackQueueThread,this ) );
  ...
  updateConnection = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&ArdroneStateController::Update, this));

  robot_current_state = LANDED_MODEL;

  // adding a random slope for bias term
  // double normMean = 0;
  // double normStd = 5.0;
  // std::normal_distribution<double> distribution(normMean, normStd);   
  biasStartTime = world->GetSimTime();
  double uniformMin = 0.01;
  double uniformMax = 0.1;
  std::uniform_real_distribution<double> distribution(uniformMin, uniformMax);
  std::default_random_engine generator;
  compassBias = distribution(generator);
  ROS_WARN("ArdroneStateController :: compassBias = % -6.4f", compassBias);
}

////////////////////////////////////////////////////////////////////////////////
// Callbacks
void ArdroneStateController::VelocityCallback(const geometry_msgs::TwistConstPtr& velocity)
{
  velocity_command_ = *velocity;
}

void ArdroneStateController::ImuCallback(const sensor_msgs::ImuConstPtr& imu)
{
...
void ArdroneStateController::SonarCallback(const sensor_msgs::RangeConstPtr& sonar_info)
{
  robot_altitude = sonar_info->range;
}


void ArdroneStateController::StateCallback(const nav_msgs::OdometryConstPtr& state)
{
...
void ArdroneStateController::Update()
{
  ...
  double biasTime = (world->GetSimTime() - biasStartTime).Double();
  navdata.rotZ = (pose.rot.GetYaw() / M_PI * 180.)+compassBias*biasTime; // this includes compass walk
  if (!sonar_topic_.empty())
...
void ArdroneStateController::Reset()
{
  // reset state
  pose.Reset();
  velocity.Set();
...
////////////////////////////////////////////////////////////////////////////////
// controller callback
void ArdroneStateController::TakeoffCallback(const std_msgs::EmptyConstPtr& msg)
{
...
void ArdroneStateController::LandCallback(const std_msgs::EmptyConstPtr& msg)
{
...
void ArdroneStateController::ResetCallback(const std_msgs::EmptyConstPtr& msg)
{
  ROS_INFO("%s","\nReset Ardrone!!");
}

bool ArdroneStateController::toggleCamCallback(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
...
void ArdroneStateController::CameraFrontCallback(const sensor_msgs::ImageConstPtr& image)
{
  if(m_selected_cam_num==0)
    camera_publisher_.publish(image);
}

void ArdroneStateController::CameraBottomCallback(const sensor_msgs::ImageConstPtr& image)
{
  if(m_selected_cam_num==1)
    camera_publisher_.publish(image);
}

void ArdroneStateController::CameraInfoFrontCallback(const sensor_msgs::CameraInfoConstPtr&  image_info)
{
  if(m_selected_cam_num==0)
    camera_info_publisher_.publish(image_info);
}

void ArdroneStateController::CameraInfoBottomCallback(const sensor_msgs::CameraInfoConstPtr&  image_info)
{
  if(m_selected_cam_num==1)
    camera_info_publisher_.publish(image_info);
...
GZ_REGISTER_MODEL_PLUGIN(ArdroneStateController)
